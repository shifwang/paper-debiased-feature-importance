...
def naive_OOB_importance(rf, X, y):
...
def cls(self, X, y):
    ...
    for index, tree in enumerate(self.estimators_):
        ...
        weighted_n_node_samples = np.array(np.sum(decision_path_oob, axis = 0))[0] / tree_X_oob.shape[0]
    ...
    for index, tree in enumerate(self.estimators_):
        ...
        for i in range(n_nodes):
            ...
            if len(arr1) == 0:
                ...
                impurity[i] = np.sum((arr1 - np.mean(arr1)) ** 2) / len(arr1)
